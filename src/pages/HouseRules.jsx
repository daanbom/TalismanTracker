...
function BannerIcon() {
  return (
    <svg
      ...
      strokeLinejoin="round"
    >
      <path d="M5 3v18l3-3 4 3 4-3 3 3V3z" />
  ...
  if (disabled) {
    return (
      <div className={`${baseClass} opacity-50 cursor-not-allowed`} aria-disabled="true">
        ...
        <p className="text-muted text-sm font-body">{disabledHint}</p>
      </div>
    )
  }
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
      <div className="mb-12 animate-fade-up text-center">
        ...
        <p className="text-muted font-body mt-4 max-w-md mx-auto">
          The fellowship's canon, your group's tweaks, and the books that came before them.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6 max-w-5xl mx-auto">
        <Tile
          index={0}
          to="/house-rules/platform"
          title="TalismanTracker House Rules"
          desc="The fellowship's canonical tweaks and clarifications"
          Icon={ScrollIcon}
        />
        <Tile
          index={1}
          to="/house-rules/group"
          title={groupName ? `${groupName} House Rules` : 'Group House Rules'}
          desc="Your group's own house rules"
          Icon={BannerIcon}
          disabled={!activeGroup}
          disabledHint="Select a group first"
        />
        <Tile
          index={2}
